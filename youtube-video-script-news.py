...
import urllib.request, time, json, json5
...
newsApiResponse = requests.get(
    "https://b.shortynews.today/getfeeds?country=CRYPTO&lang=en"
)

newsText = newsApiResponse.json().get("data")[1].get("text")[0]

# print(type(newsText), newsText)
    ...
    if AI == "OPENAI":
        chat_completion = client.chat.completions.create(
            messages=[
                {
                    "role": "system",
                    "content": "You are an expert short form video news generator, your content is straight to the point, and very controversial in nature.",
                },
                {
                    "role": "user",
                    "content": "Write the audio script (min: 800 characters long, max 1000 characters long) for a video from given news article snippet. Use TikTok news video style script tone. Start from main shocking point from the news (shocking statement). Skip opening hi/hello welcome kind of statements, directly jump to the story which will keep the user hooked. Try to pack the maximum amount of mind blowing information in the script as possible. Use “Global English” to make content and context accessible for non-native comprehension. Don’t use idioms. Be literal and stay away from metaphors and colloquial language. Keep sentences short. Standardise terminology to minimise changes. Avoid directional language. Use inclusive, accessible, person-first language. This audio script will be further fed into TTS engine so write accordingly. Also return seo title, seo description and seo hashtags (only 3 tags) for youtube uploads. Keep title very very short.\nReturn your answer strictly in this json format: { 'script': '', seoTitle: '', seoDescription: '', seoHashtags: '' }\n\n\nNews article snippet: "
                    + newsText
                    + "",
                },
            ],
        ...
        convo = geminiModel.start_chat(history=[])
        convo.send_message(
            "You are an expert short form viral video news generator, your content is straight to the point, and always goes viral. Write a viral audio script (min: 800 characters long, max 1000 characters long) for a video from given news article snippet. Use TikTok news video style script tone. Start from main shocking point from the news (shocking statement). Skip opening hi/hello welcome kind of statements, directly jump to the story which will keep the user hooked. Try to pack the maximum amount of mind blowing information in the script as possible. Use “Global English” to make content and context accessible for non-native comprehension. Don’t use idioms. Be literal and stay away from metaphors and colloquial language. Keep sentences short. Standardise terminology to minimise changes. Avoid directional language. Use inclusive, accessible, person-first language. This audio script will be further fed into TTS engine so write accordingly. Also return viral seo title, seo description and seo hashtags (only 3 tags) for youtube uploads. Keep title very very short.\nReturn your answer strictly in this json format: { 'script': '', seoTitle: '', seoDescription: '', seoHashtags: '' }\n\n\nNews article snippet: "
            + newsText
            + ""
        )
        print("\n************************\n")
        ...
        print("************************\n")
        content = json5.loads(convo.last.text)
...
if videoType == "VIDEOS":
    ...
    for entry in videoTags:
        ...
        command = f"/home/sumit/.nvm/versions/node/v16.20.2/bin/node utils/getLexicaImages.js {prompt}"
...
run_shell_command(
    "/home/sumit/.nvm/versions/node/v16.20.2/bin/node utils/youtube-upload.js run"
)
