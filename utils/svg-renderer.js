...
class SvgRenderer {
  ...
  _generateLineNumber(lineNumber) {
    ...
    return `<tspan fill="${
      this._lineNumberForeground
    }" fill-opacity="0.25" x="-${this._fontWidth * 4}">${content}</tspan>`;
  }

  _getTokenSVGAttributes(token) {
  ...
  renderToSVG(lines) {
    ...
    for (const lineTokens of lines) {
      const lineTextLength = lineTokens.reduce(
        (sum, current) => (sum += current.content.length),
        0
      );
    ...
    const lineNumberWidth =
      (this._showLineNumber ? totalLines : 2) * this._fontWidth;
    ...
    const backgroundHeight =
      (lines.length + 1) * this._lineHeight + titlebarHeight;
      ...
      if (isLineEmpty) {
        ...
        for (const token of line) {
          ...
          if (
            ...
            }" ${tokenAttributes}>${whitespace}</tspan>`;

            // render content
            const escapedContent = this._escapeHTML(
